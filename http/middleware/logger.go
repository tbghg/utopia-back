...
func Logger() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		logFields := []zap.Field{
			...
			zap.String("time", cast.ToString(cost)),
		}
		...
		if responseStatus > 400 && responseStatus <= 499 {
			...
			logger.Logger.Warn("HTTP Warning "+cast.ToString(responseStatus), logFields...)
		} else if responseStatus >= 500 && responseStatus <= 599 {
			// 除了内部错误，记录 error
			logger.Logger.Error("HTTP Error "+cast.ToString(responseStatus), logFields...)
		} else {
			logger.Logger.Debug("HTTP Access Log", logFields...)
		}
	}
}
