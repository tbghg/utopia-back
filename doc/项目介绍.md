...
+ 使用七牛云 kodo 存储文件，采用 uploadToken + 回调 确保密钥安全性
+ 针对未上传封面的视频，异步截取视频首帧，回调替换视频封面。

详见：[七牛云SDK使用](./七牛云SDK使用.md)

## 中间件介绍

+ 限频中间件：采用 令牌桶 对可针对不同接口配置不同的限流策略，同时支持对用户IP限流，防止用户恶意攻击，详见：[接口及IP限流](./接口及IP限流.md)
+ 鉴权模块：通过 JWT 进行用户鉴权，user_id 作 payload
+ 参数校验：通过 validator 库对参数进行校验
+ 日志：使用 Zap 重写 Gorm 和 Gin 日志，提供更好的日志输出控制和格式化选项，以及更好的日志级别管理和错误追踪能力
...
通过 webhook + hookdoo + 自定义脚本 实现持续部署，提高开发效率。详见：[自动部署方案](./自动部署方案.md)
